...
st.set_page_config(page_title="Series Trends | Conference Analysis", page_icon="\U0001F4C8", layout="wide")

st.title("Series Trends")
st.caption(
    "Year-over-year performance for a recurring conference series — "
    "is this conference improving, flat, or declining as a source?"
)
...
if series_df.empty:
    st.warning("No conference series with 2+ years of data match the current filters.")
    st.stop()
...
yearly_df["HQT_CONVERSION_RATE"] = (yearly_df["HQTS_SOURCED"] / yearly_df["COMPANIES_MET"]).where(
    yearly_df["COMPANIES_MET"] > 0
)
yearly_df["HQM_CONVERSION_RATE"] = (yearly_df["HQTS_AT_HQM"] / yearly_df["HQTS_SOURCED"]).where(
    yearly_df["HQTS_SOURCED"] > 0
)
